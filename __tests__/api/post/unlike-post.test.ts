...
describe("API - unlikePost", async () => {
  ...
  await runWithUser(async (headers) => {
    it("should raise an error if post is not found", async () => {
      const response = await auth.api.unlikePost({
        body: {
    ...
    it("should return an updated likes count if post is found and liked by the user", async () => {
      ...
      const responseUnlike = await auth.api.unlikePost({
        body: {
          postId: post.id,
        },
        headers,
      ...
      const bodyUnlike = await responseUnlike.json();
      expect(bodyUnlike.post).toBeDefined();
    ...
    it("should return an updated likes count if post is liked by a foreign user", async () => {
      ...
      const bodyUnlike = await responseUnlike.json();
      expect(bodyUnlike.post).toBeDefined();
      expect(bodyUnlike.post.likesCount).toBe(1);

      const responseSecondUnlike = await auth.api.unlikePost({
        body: {
          postId: post.id,
        },
        headers,
      ...
      const bodySecondUnlike = await responseSecondUnlike.json();
      expect(bodySecondUnlike.post).toBeDefined();
      expect(bodySecondUnlike.post.likesCount).toBe(0);
    });
  });
});