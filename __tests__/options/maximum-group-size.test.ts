    ...
    describe("with maxGroupSize set to 3", async () => {
      ...
      await runWithUser(async (headers) => {
          ...
          await Promise.all(
            Array.from({ length: 3 }).map(async (_, index) => {
              return auth.api
                ...
                .then(
                  async ({ user: foreignUser, token: tokenForeignUser }) =>
                    await auth.api
                      .sendFriendRequest({
                        body: {
                      ...
                      .then(
                        async ({ friendRequest }) =>
                          await auth.api
                            ...
                            .then(
                              async ({ success: acceptFriendRequestSuccess }) =>
        ...
        it("should raise an error if trying to create a group chat with more than the maximum size", async () => {
          ...
          expect(body.code).toBe(SOCIAL_NETWORK_ERROR_CODES.MAX_GROUP_SIZE.code);
          expect(body.message).toBe(SOCIAL_NETWORK_ERROR_CODES.MAX_GROUP_SIZE.message);
        });
