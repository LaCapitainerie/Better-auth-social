  ...
  describe("option set to true", async () => {
    ...
    await runWithUser(async (headers) => {
      ...
      it("creating a group chat with an unknown member should work when the foreignUser is not a friend", async () => {
        const { user: unknownUser, token: tokenUnknownUser } =
          await auth.api.signUpEmail({
            body: {
              name: "Unknown User",
        ...
        expect(members.find((m) => m.userId === unknownUser.id)?.role).toBe(
          "member",
        );
      });
      ...
      it("creating a group chat with an unknown member should work when the foreignUser is a friend", async () => {
        ...
        expect(members.find((m) => m.userId === user.id)?.role).toBe("admin");
        expect(
          members.find((m) => m.userId === foreignUser.friendId)?.role,
        ).toBe("member");
      });
      ...
      it("adding an unknown member to a group chat should work when the foreignUser is not a friend", async () => {
        ...
        const { user: unknownUser, token: tokenUnknownUser } =
          await auth.api.signUpEmail({
            body: {
              name: "Unknown User",
              email:
                "unknown-user-option-true-add-member-to-group-chat@example.com",
              password: "password",
            },
          });
        expect(unknownUser).toBeDefined();
        ...
        const { success: addMemberToGroupChatSuccess } =
          await auth.api.addMemberToGroupChat({
            body: {
              groupChatId: groupChat.id,
      ...
      it("adding an unknown member to a group chat should work when the foreignUser is a friend", async () => {
        ...
        expect(initialMembers.find((m) => m.userId === user.id)?.role).toBe(
          "admin",
        );
        ...
        expect(
          updatedMembers.find((m) => m.userId === foreignUser.friendId)?.role,
        ).toBe("member");
      });
    });
  });
    ...
    await runWithUser(async (headers) => {
      ...
      it("creating a group chat with an unknown member should raise an error when the foreignUser is not a friend", async () => {
        ...
        expect(body.code).toBe(
          SOCIAL_NETWORK_ERROR_CODES.ADDING_UNKNOWN_MEMBERS_TO_GROUP_CHAT.code,
        );
        expect(body.message).toBe(
          SOCIAL_NETWORK_ERROR_CODES.ADDING_UNKNOWN_MEMBERS_TO_GROUP_CHAT
            .message,
        );
      });
        ...
        const { user: unknownUser, token: tokenUnknownUser } =
          await auth.api.signUpEmail({
            body: {
              name: "Unknown User",
              email:
                "unknown-user-option-false-add-member-to-group-chat@example.com",
              password: "password",
            },
          });
        expect(unknownUser).toBeDefined();
