    ...
    await runWithUser(async (headers) => {
      ...
      it("should return an error when the sender try to delete a message but the option is set to CANT_DELETE", async () => {
        ...
        expect(body.code).toBe(
          SOCIAL_NETWORK_ERROR_CODES.MESSAGE_DELETION_RULE_CANT_DELETE.code,
        );
        expect(body.message).toBe(
          SOCIAL_NETWORK_ERROR_CODES.MESSAGE_DELETION_RULE_CANT_DELETE.message,
        );
  ...
  describe("with messageDeletionRule set to SENDER_ONLY_VISIBLE", async () => {
    ...
    await runWithUser(async (headers) => {
      beforeAll(async () => {
        ...
        const { success: deleteMessageSuccess } =
          await auth.api.deleteGroupChatMessage({
            body: {
              groupChatId: groupChat.id,
              messageId: message.id,
            },
